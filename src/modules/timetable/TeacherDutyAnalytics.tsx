...
export const TeacherDutyAnalytics: React.FC<TeacherDutyAnalyticsProps> = ({
  ...
  const [activeSubTab, setActiveSubTab] = useState<'leaderboard' | 'arrival_radar'>('arrival_radar');
  ...
  // Real-time arrival radar stats
  const completedDuties = roundDuties.filter((r) => r.status === 'Completed' || r.status === 'Checked In');
  const delayedDuties = roundDuties.filter((r) => r.remarks && r.remarks.includes('Observation'));
  const onTimeDuties = completedDuties.filter((r) => !r.remarks || !r.remarks.includes('Observation'));
  const pendingDuties = roundDuties.filter((r) => r.status === 'Assigned');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white p-6 rounded-2xl border border-slate-800 shadow-xl space-y-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                Live QR Arrival & Workload Radar
              </span>
            </div>
            <h3 className="text-xl font-black text-white mt-1 flex items-center gap-2">
              <Award className="w-6 h-6 text-amber-400" />
              Teacher Round Duty, QR Arrival & Annual Substitution Analytics
            </h3>
            <p className="text-xs text-slate-300 mt-1">
              Live mobile QR scan updates inform the Principal which teachers have arrived on time, duty completion logs, and ranked annual workload summary.
            </p>
          </div>

          <div className="flex items-center gap-2">
            <button
              onClick={() => setActiveSubTab(activeSubTab === 'arrival_radar' ? 'leaderboard' : 'arrival_radar')}
              className="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-white font-bold text-xs rounded-xl shadow cursor-pointer border border-slate-700 transition-all flex items-center gap-2 shrink-0"
            >
              <Eye className="w-4 h-4 text-indigo-400" />
              <span>{activeSubTab === 'arrival_radar' ? 'View Annual Leaderboard' : 'View Live Arrival Radar'}</span>
            </button>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1">
          <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800 flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-extrabold text-slate-400 uppercase">On-Time QR Check-Ins</span>
              <div className="text-xl font-black text-emerald-400">{onTimeDuties.length} Verified</div>
            </div>
          </div>
          ...
          <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800 flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-extrabold text-slate-400 uppercase">Delay / Remarks Alerts</span>
              <div className="text-xl font-black text-amber-300">{delayedDuties.length} Reported</div>
            </div>
          </div>
          ...
          <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800 flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-extrabold text-slate-400 uppercase">Substitutions Active</span>
              <div className="text-xl font-black text-cyan-300">{totalSubstitutionsTaken} Classes</div>
            </div>
          </div>
          ...
          <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800 flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-extrabold text-slate-400 uppercase">Pending Patrols</span>
              <div className="text-xl font-black text-indigo-300">{pendingDuties.length} Scheduled</div>
            </div>
          </div>
        </div>
      </div>

      {/* VIEW 1: PRINCIPAL'S LIVE ARRIVAL & ROUND DUTY RADAR */}
      {activeSubTab === 'arrival_radar' && (
          ...
          <div className="p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="text-sm font-black text-slate-900 dark:text-white">
                    Principal's Live Teacher Arrival & Patrol Radar
                  </h4>
                  <p className="text-xs text-slate-500">
                    Real-time status of teachers updating round duties via mobile QR scanner.
                  </p>
                </div>
              </div>
              ...
              <span className="px-3 py-1 rounded-full text-xs font-black bg-indigo-50 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300 border border-indigo-200">
                Live Campus Radar Operational
              </span>
            </div>

            {/* Radar Audit List */}
            <div className="space-y-2">
              {roundDuties.length === 0 ? (
                <p className="text-xs text-slate-400 italic text-center p-6">
                  No round duties scheduled in system.
                </p>
              ) : (
                roundDuties.map((duty) => {
                  const isDone = duty.status === 'Completed' || duty.status === 'Checked In';
                  const hasRemark = duty.remarks && duty.remarks.includes('Observation');
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className="px-2 py-0.5 rounded-md text-[10px] font-black bg-indigo-600 text-white">
                            Period #{duty.periodNumber} ({duty.timeSlot})
                          </span>
                        ...
                        {isDone ? (
                          <div className="text-right">
                            <span className={`px-3 py-1 rounded-xl text-xs font-black border flex items-center gap-1.5 ${
                              ...
                              <span>{hasRemark ? 'Checked In • Alert' : 'Arrived On Time'}</span>
                            </span>
                            <span className="text-[10px] font-mono text-slate-400 block mt-0.5">
                              QR Scanned at {duty.checkInTime || '08:45 AM'}
                            </span>
                          </div>
                        ) : (
                          <span className="px-3 py-1 rounded-xl text-xs font-bold bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-300">
                            ⏳ Pending Check-In
                          </span>
                        )}
                      </div>
      ...
      {/* VIEW 2: ANNUAL LEADERBOARD RANKING */}
