...
import { useAdmissionNumberConfig } from './admissionNumberConfig';
...
import {
  ...
  EntranceTestResultModal,
  AdminNumberingConfigModal
...
export const AdmissionModule: React.FC = () => {
  ...
  const {
    ...
    updateInquirySeries,
    resetToDefaults: resetNumberingDefaults
  ...
  const [showTestModal, setShowTestModal] = useState<AdmissionApplication | null>(null);
  ...
  const handleAdmissionStepSubmit = (registrationId: string, studentCategory: StudentCategoryType, details: any) => {
    promoteRegistrationToAdmission(registrationId, studentCategory, details);
  ...
  const handleSaveEditedRecord = (applicationId: string, auditLog: any, updatedFields?: Partial<AdmissionApplication>) => {
    addProtectedEditLog(applicationId, auditLog, updatedFields);
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <button
                  ...
                  title="Configure non-editable ID & Number Series formats (Admin Only)"
                >
                  <Hash className="w-3.5 h-3.5 text-indigo-600" /> ID Series Config
                </button>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-800 text-[11px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                    ...
                    <th className="py-3 px-4">Class & Prev. Class</th>
                    <th className="py-3 px-4">Parent Details</th>
                    <th className="py-3 px-4">Joining & Start Month</th>
                    ...
                    return (
                      <tr key={app.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                        <td className="py-3 px-4">
                          ...
                          <span className="inline-block mt-0.5 px-2 py-0.5 rounded-md text-[10px] font-bold bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
                            {app.studentCategory || 'Day Scholar'}
                          </span>
                        </td>

                        <td className="py-3 px-4">
                          <p className="font-extrabold text-slate-800 dark:text-slate-200">{app.applyingClass}</p>
                          <p className="text-[10px] text-slate-500">
                            Prev: <span className="font-medium text-slate-700 dark:text-slate-300">{app.previousSchoolClass || 'Playgroup (PG)'}</span>
                          </p>
                        </td>
                        ...
                        <td className="py-3 px-4">
                          <p className="font-bold text-slate-800 dark:text-slate-200">{app.dateOfJoining || '2026-04-01'}</p>
                          <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold">
                            Start: {app.feeApplicableFromMonth || 'April'}
                          </p>
                        </td>
                        ...
                        <td className="py-3 px-4">
                          <button
                            type="button"
                            onClick={() => setShowTestModal(app)}
                            className="text-left group cursor-pointer"
                            title="Click to input or edit test score"
                          >
                            {app.entranceTestScore !== undefined ? (
                              <div className="flex items-center gap-1.5 font-mono font-bold text-slate-900 dark:text-white">
                                <span className="px-2 py-0.5 rounded bg-purple-100 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300 border border-purple-300 dark:border-purple-800 text-[11px]">
                                  {app.entranceTestScore} / {app.entranceTestMaxMarks || 40}
                                </span>
                              </div>
                            ) : (
                              <span className="px-2 py-0.5 rounded text-[10px] font-extrabold bg-slate-100 dark:bg-slate-800 text-slate-500 border border-slate-200 dark:border-slate-700 group-hover:border-indigo-400 group-hover:text-indigo-600 transition-colors">
                                + Input Test Score
                              </span>
                            )}
                          </button>
                        ...
                        <td className="py-3 px-4 text-right space-x-1.5">
                          <button
                            ...
                            className="px-2 py-1 text-xs font-bold text-slate-700 dark:text-slate-300 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 rounded-lg cursor-pointer transition-all inline-flex items-center gap-1 border border-slate-200 dark:border-slate-700"
                          ...
                          <button
                            onClick={() => setShowTestModal(app)}
                            className="px-2 py-1 text-xs font-bold text-purple-700 dark:text-purple-300 bg-purple-50 dark:bg-purple-950/60 hover:bg-purple-100 rounded-lg cursor-pointer transition-all inline-flex items-center gap-1 border border-purple-200 dark:border-purple-800"
                            title="Record Entrance Test Result"
                          >
                            <Award className="w-3.5 h-3.5 text-purple-600" /> Test Marks
                          </button>

                          <button
                            onClick={() => setShowAllocationModal(app)}
                            className="px-2 py-1 text-xs font-bold text-amber-800 dark:text-amber-200 bg-amber-50 dark:bg-amber-950/60 hover:bg-amber-100 rounded-lg cursor-pointer transition-all inline-flex items-center gap-1 border border-amber-200 dark:border-amber-800"
                          ...
                          <button
                            ...
                            className="px-2.5 py-1 text-xs font-bold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/60 hover:bg-indigo-100 rounded-lg cursor-pointer transition-all inline-flex items-center gap-1 border border-indigo-200 dark:border-indigo-800"
                            title="Generate & Save Provisional Offer Letter (With SIS Directory Sync option)"
                          >
                          ...
                          <button
                            ...
                            className="px-2 py-1 text-xs font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/60 hover:bg-emerald-100 rounded-lg cursor-pointer transition-all inline-flex items-center gap-1 border border-emerald-200 dark:border-emerald-800"
      ...
      {/* ENTRANCE TEST RESULT MODAL */}
      {showTestModal && (
        <EntranceTestResultModal
          application={showTestModal}
          onClose={() => setShowTestModal(null)}
          onSave={(appId, testData) => {
            saveEntranceTestResult(appId, testData);
          }}
          onProceedToOfferLetter={(app) => {
            setShowOfferModal(app);
          }}
        />
      )}

      {/* ADMIN NUMBERING SERIES CONFIG MODAL */}
      {showNumberingModal && (
        <AdminNumberingConfigModal
          settings={numberingSettings}
          onClose={() => setShowNumberingModal(false)}
          onSaveAdmissionSeries={updateAdmissionSeries}
          onSaveRegistrationSeries={updateRegistrationSeries}
          onSaveInquirySeries={updateInquirySeries}
          onReset={resetNumberingDefaults}
        />
      )}
      ...
      {showAdmissionStepModal && (
        <AddAdmissionStepModal
          registrations={registeredCandidates}
          onClose={() => setShowAdmissionStepModal(false)}
          onSubmit={handleAdmissionStepSubmit}
          onOpenTestModal={(app) => setShowTestModal(app)}
        />
      )}
